...
export default {
  ...
  watch: {
    category(val) {
      this.SET_TREE_DATA([{ name: val.category.parent?.name, slug: val.category.parent?.slug, id: val.category.parent?.id, children: [val.category]}]);
    }
  },
  mounted() {
    this.getCategory(this.$route.params.subCategory);
  },
}
</script>
<style lang="">
