<template>
  <div
    ...
    <div class="col-span-10 border-l border-grey-3 pl-8">
      <nuxt-link :to="`/category/${category.slug}`" class="text-4xl font-semibold w-fit text-black hover:text-orange block mb-10">{{ category.name }}</nuxt-link>
      <div class="grid grid-cols-4 gap-8">
        <div v-for="subCategory in category.children" :key="subCategory.id">
          <nuxt-link :to="`/category/${category.slug}/${subCategory.slug}`" class="w-fit font-medium text-black hover:text-orange block text-xl mb-6">{{ subCategory.name }}</nuxt-link>
          <ul class="list-none ml-2 flex flex-col text-black-2 gap-2 text-base">
            <li v-for="subSubCategory in subCategory?.children" :key="subSubCategory.id"><nuxt-link class="text-grey-text hover:text-orange" :to="`/category/${category.slug}/${subCategory.slug}/${subSubCategory.slug}`">-{{ subSubCategory.name }}</nuxt-link></li>
