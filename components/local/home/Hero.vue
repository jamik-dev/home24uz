        ...
        <div class="w-full h-full">
          <client-only>
            <customCarousel :arrows="true" class="rounded-3xl">
              <img v-for="item in 3" :key="item" class="h-full" src="~/assets/img/banner/1.png" alt="banner">
            </customCarousel>
          </client-only>
        </div>
      </div>
            ...
            <figure v-for="product in day_products" :key="product.products[0].id" class="px-8 py-7">
              ...
              <div class="flex gap-4 mt-8 items-center">
                <div class="w-1/2 h-[246px]">
                  <img class="h-full w-full object-cover" :src="product.products[0].images[0].lg_img || `~/assets/img/chair/1_cropped.png`" :alt="product.products[0].for_search">
                </div>
                <div class="w-1/2">
                  <div class="rounded-xl overflow-hidden relative w-[150px] px-[10px] py-1 bg-grey-light"><span
                    ...
                    <div
                      ...
                      <span v-if="product.products[0].discount" class="text-xl font-bold text-white">-{{product.products[0].discount}}%</span>
                  ...
                  <div class="mt-4">
                    ...
                    <p v-if="product.products[0].discount_price" class="font-ttfirs line-through text-[16px] text-grey-text">{{ product.products[0].discount_price }}</p>
                  </div>
                  ...
                  <div class="mt-3">
                    <nuxt-link :to="`/category/nimadir/nimadir/${product.products[0].slug}`" class="text-lg text-grey-text hover:text-orange leading-tight line-clamp-3">{{ product.products[0].name }}</nuxt-link>
                  </div>
                  <button
